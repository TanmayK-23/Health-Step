  ...
  if(loginBtn){
    loginBtn.addEventListener('click', async ()=>{
      ...
      try {
        ...
        if (em.includes('SERVER CONNECTION FAILED') || em.includes('FETCH') || em.includes('NETWORK') || em.includes('ABORT') || em.includes('404') || em.includes('502') || em.includes('503') || em.includes('504')) {
...
const API = {
  ...
  async json(res){
    ...
    if (
      ...
      throw new Error('Server connection failed. Please check your network and try again.');
    }
...
async function loadChallenges() {
  ...
  try {
    const [active, joined] = await Promise.all([
      API.get('/challenges/active').catch(()=>[]),
      API.get(`/challenges/user/${uid}`).catch(()=>[])
    ]);

    const activeList = document.getElementById('active-challenges-list');
    const myList = document.getElementById('my-challenges-list');
    if (!activeList || !myList) return;

    // Render active
    if (active && active.length) {
      activeList.innerHTML = active.map(c => `
        <div class="nutri-panel" style="background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); border-radius:12px; padding:16px;">
          <h3 style="margin:0 0 8px; font-size:18px; color:#f59e0b;">${c.title}</h3>
          <p style="font-size:14px; color:#cbd5e1; margin-bottom:12px;">${c.description}</p>
          <button class="btn btn-join-challenge" data-id="${c.id}" style="padding:8px 16px;">Join Challenge</button>
        </div>
      `).join('');
    } else {
      activeList.innerHTML = '<p style="color:#9ca3af;">No active challenges right now.</p>';
    }

    // Render my challenges
    if (joined && joined.length) {
      const challengeMap = new Map();
      if (active) active.forEach(c => challengeMap.set(c.id, c));
      
      myList.innerHTML = joined.map(uc => {
        const c = challengeMap.get(uc.challengeId);
        if (!c) return '';
        const pct = Math.min(100, Math.round((uc.progress / c.targetValue) * 100));
        const color = uc.completed ? '#10b981' : '#3b82f6';
        return `
        <div style="background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); border-radius:10px; padding:12px;">
          <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
            <strong style="color:#e5e7eb; font-size:14px;">${c.title}</strong>
            <span style="color:#9ca3af; font-size:12px;">${uc.progress} / ${c.targetValue}</span>
          </div>
      ...
      myList.innerHTML = '<p style="color:#9ca3af; font-size:14px;">You haven\'t joined any challenges yet.</p>';
    }
    ...
    document.querySelectorAll('.btn-join-challenge').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          await API.post(`/challenges/${cid}/join/${uid}`);
          loadChallenges(); // reload
        } catch (err) {
          window.__HS_API_ERR__?.show('Failed to join challenge.');
        }
      });
    });
